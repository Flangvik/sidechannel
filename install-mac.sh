...
# Get phone number
echo -e "  Enter your phone number in E.164 format (+ country code + number):"
echo -e "  Examples: ${CYAN}+12025551234${NC} (US/CA)  ${CYAN}+447911123456${NC} (UK)  ${CYAN}+4915212345678${NC} (DE)"
echo -e "            ${CYAN}+33612345678${NC} (FR)    ${CYAN}+61412345678${NC} (AU)   ${CYAN}+819012345678${NC} (JP)"
read -p "  > " PHONE_NUMBER

if [ -n "$PHONE_NUMBER" ]; then
    if [[ ! "$PHONE_NUMBER" =~ ^\+[1-9][0-9]{6,14}$ ]]; then
        echo -e "  ${YELLOW}Warning: doesn't look like E.164 format (e.g., +12025551234 or +447911123456)${NC}"
        read -p "  Continue anyway? [y/N] " -n 1 -r
        echo
        if [[ ! $REPLY =~ ^[Yy]$ ]]; then
